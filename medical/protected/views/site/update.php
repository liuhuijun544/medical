...
<div class="layui-main">
	...
	<form class="layui-form" action="/index.php/site/updatePost" method="post">
	<div class="content-content">
	<div class="content-left">
			<div class="layui-form-item" style="padding-top:20px">
		    	<label class="layui-form-label">病人姓名：</label>
		    	<div class="layui-input-block">
			     	<input type="text" name="patientName" lay-filter="patientName"  lay-verify="required" placeholder="请输入姓名" autocomplete="off" class="layui-input patientName" value="<?php echo $prescription[0]->pname?$prescription[0]->pname:'';?>">
		  	...
		  	<div class="layui-form-item">
			    ...
			    <div class="layui-input-block">
			      <input type="radio" name="sex" id=sex1 value="1" title="男" <?php if ($prescription[0]->sex==1){ echo "checked";}?>>
			      <input type="radio" name="sex" id=sex2 value="2" title="女" <?php if ($prescription[0]->sex==2){ echo "checked";}?>>
			    </div>
		  	</div>
		  	<div class="layui-form-item">
		    	<label class="layui-form-label">年龄：</label>
		    	<div class="layui-input-block">
			     	<input type="text" name="age" lay-filter="age"  lay-verify="required|number" placeholder="请输入年龄" autocomplete="off" class="layui-input age" value="<?php echo $prescription[0]->age?$prescription[0]->age:''?>">
			    </div>
		  	</div>
		  	<div class="layui-form-item">
				<label class="layui-form-label">家庭地址：</label>
			    <div class="layui-input-block">
			     	<input type="text" name="address" lay-filter="address" placeholder="请输入家庭地址" autocomplete="off" class="layui-input address" value="<?php echo $prescription[0]->address?$prescription[0]->address:''?>">
			    </div>
			</div>
		  	<div class="layui-form-item layui-form-text">
				<label class="layui-form-label">病名：</label>
			    <div class="layui-input-block">
			    	<textarea name="etiology" placeholder="请输入病名" lay-verify="required" class="layui-textarea"><?php echo $prescription[0]->etiology?$prescription[0]->etiology:''?></textarea>
			    </div>
			</div>
			<div class="layui-form-item">
				<label class="layui-form-label">服药方式：</label>
			    <div class="layui-input-block">
			     	<input type="text" name="way" lay-filter="way" placeholder="请输入服药方式" autocomplete="off" class="layui-input way" value="<?php echo $prescription[0]->way?$prescription[0]->way:''?>">
			    </div>
			</div>
			<div class="layui-form-item">
				<label class="layui-form-label">总价格：</label>
			    <div class="layui-input-block">
			     	<input type="text" name="totalMoney" lay-verify="totalMoney" lay-filter="totalMoney" placeholder="请输入总价" autocomplete="off" class="layui-input totalMoney" value="<?php echo $prescription[0]->totalMoney?$prescription[0]->totalMoney:''?>">
			     	<input type="hidden" name="main_id" value="<?php echo $prescription[0]->main_id;?>">
			    </div>
			</div>
		  	<div class="layui-form-item">
		    	<div class="layui-input-block">
...
<!-- 		      		<button type="reset" class="layui-btn layui-btn-primary">重置</button> -->
	...
	<div class="content-right">
		<table class="layui-table" style="margin-top: 0px;">
			  ...
			  <tbody id="tbody">
			  <?php for ($i=1;$i<=count($prescription);$i++){?>
					<tr>
						<td class="listNum"><?php echo $i?>
							<input type="hidden" name=detail_id[] value="<?php echo $prescription[$i-1]->id?>">
						</td>
						...
						<td style="padding:9px 5px;">
							<div id="midicineselect<?php echo $i?>" style="float:left; display:inline;position: relative;width:150px;margin-right:-23px;">
								<input type="text" id="meidicine<?php echo $i?>" style="width:150px;" lay-verify="td_medicine"  value="<?php echo $prescription[$i-1]->mname?>" />
								<input type='hidden' id='medical<?php echo $i?>'  name="medicine[]" class="td_medicine" value="<?php echo $prescription[$i-1]->mid?>"/>
							</div>
						...
						<td><input type="text" name=weight[] lay-verify="td_weight"  class="layui-input weight" value="<?php echo $prescription[$i-1]->weight?>"></td>
						<td><input type="text" name=amount[] lay-verify="td_amount"  class="layui-input amount" value="<?php echo $prescription[$i-1]->amount?>"></td>
						<td><input type="text" name=price[] lay-verify="td_price"  class="layui-input price" value="<?php echo $prescription[$i-1]->pdprice?>"></td>
						<td><input type="text" name=total[] lay-verify="td_total"  class="layui-input total" readonly value="<?php echo $prescription[$i-1]->total?>"></td>
					</tr>
				<?php }?>
			</tbody>
			<tfoot>
...
layui.use(['layer', 'form'], function(){
	...
	$(".addRow").click(function(){
		...
		var newRow = '<tr><td class="listNum">'+count+'<input type="hidden" name=detail_id[] value=0></td>'+
			'<td><i class="layui-icon del_tr"  style="font-size: 25px;cursor: pointer;">&#xe640;</i></td>'+
			'<td style="padding:9px 5px;">'+
