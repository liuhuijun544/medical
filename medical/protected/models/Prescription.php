...
class Prescription extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('patient_id, created_at, created_by', 'numerical', 'integerOnly'=>true),
			array('etiology', 'length', 'max'=>500),
			array('way', 'length', 'max'=>255),
			array('totalMoney', 'length', 'max'=>11),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, patient_id, etiology, created_at, created_by, way, totalMoney', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			"detail" => array(self::HAS_MANY, 'PrescriptionDetail', 'prescription_id'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('totalMoney',$this->totalMoney,true);
